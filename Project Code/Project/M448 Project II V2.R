...
glm.fit1 = glm(call~ethnicity+experience+honors+holes+special+reqorg+gender+college+industry, train.data, family = "binomial")
summary(glm.fit1)
...
qda.fit = qda(call~ethnicity+experience+honors+holes+special+reqorg+gender+college+industry, data =  train.data)
qda.fit
...
lda.fit = lda(call~ethnicity+experience+honors+holes+special+reqorg+gender+college+industry, data = train.data) 
lda.fit
plot(lda.fit)
...
regfit.full = regsubsets(call~ethnicity+experience+honors+holes+special+reqorg+gender+college+industry, data = Names, nvmax = 20)
...
points(8, reg.summary$adjr2[8], col = "purple", cex = 2, pch = 20)
plot(reg.summary$cp, xlab = "Number of Variables", ylab = "Cp",type='l')
which.min(reg.summary$cp)
points(6, reg.summary$cp[6], col = "purple", cex = 2, pch = 20)
which.min(reg.summary$bic)
plot(reg.summary$bic, xlab = "Number of Variables", ylab = "BIC", type = 'l')
points(2, reg.summary$bic[2], col = "purple", cex = 2, pch = 20)

############################
# Ridge Regression & Lasso #
############################

x = model.matrix(call ~ ethnicity+experience+honors+holes+special+reqorg+gender+college+industry)[,-1]
y = Names$call
dim(x)
